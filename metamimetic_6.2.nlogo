...
to setup
  ...
  set-outputs
  ifelse timescale [set final-time 1200][set final-time 100]
...
to stabilization
  ...
  ask turtles [select-behavior]    
    ...
    repeat 10 [ask turtles[
        if not am-i-the-best? and any? turtles-on neighbors [select-rule]
        ]]
...
to set-outputs
  ...
  set move-rate count turtles with [move?] / (count turtles with [move? or rule? or behavior?]+ 1)
  set rule-rate count turtles with [rule?] / (count turtles with [move? or rule? or behavior?]+ 1)
  set behavior-rate count turtles with [behavior?] / (count turtles with [move? or rule? or behavior?] + 1)
...
to do-plots
  ...
  histogram [theta_2 / theta_1] of turtles 
  
   set-current-plot-pen "theta_2"
  set-histogram-num-bars 1000
  histogram [theta_1] of turtles 
...
;    )
   ...
   let temp-dist []
    foreach life-distribution [
      let month-factor (1 - (1 - ?) ^ 0.125)
...
to select-rule
       ...
       [copy-strategy agent-after-reflect]       
       [copy-strategy (one-of best-elements)]
       
end
to-report agent-after-reflect
  let test-agent self
  let test-score score
    ...
    if rule = 3 [
      ...
      if test-rule != 3 [set test-agent one-of (turtles-on neighbors) with [rule = test-rule]]
      ]  
    if rule = 4 [
      ...
      if test-rule != 4 [set test-agent one-of (turtles-on neighbors) with [rule = test-rule]]
...
to plot-rt-hist
  set-current-plot "rate_theta"
  set-current-plot-pen "rate-theta"
  plot-pen-reset
  ;foreach array:to-list stime-rt [ plot ? ]
